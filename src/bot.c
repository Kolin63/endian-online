...
void bot_init(struct bot* bot, struct cli_args* cli_args) {
  ...
  if (cli_args->verbose == 0) {
    ...
    logmod_logger_set_quiet(
        logmod_get_logger(discord_get_logmod(bot->discord_bot), "HTTP_RAW"), 1);
    logmod_logger_set_quiet(
        logmod_get_logger(discord_get_logmod(bot->discord_bot), "GATEWAY"), 1);
    logmod_logger_set_quiet(
        logmod_get_logger(discord_get_logmod(bot->discord_bot), "REQUEST"), 1);
    logmod_logger_set_quiet(
