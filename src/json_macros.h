...
#define END_JSON_CHECK_BOOL                                                \
  if (iter->json->type != cJSON_False && iter->json->type != cJSON_True) { \
    log_error("In data type %s from mod %s, %s must be bool", file_name,   \
...
#define END_JSON_CHECK_NUMBER                                              \
  if (iter->json->type == cJSON_Number) {                                  \
    log_error("In data type %s from mod %s, %s must be number", file_name, \
              mod_name, iter->json->string);                               \
    error++;                                                               \
    continue;                                                              \
  }
...
#define END_JSON_CHECK_INT                                                  \
  ...
  if ((int)iter->json->valuedouble != iter->json->valuedouble) {            \
    log_error("In data type %s from mod %s, %s must be integer", file_name, \
              mod_name, iter->json->string);                                \
    error++;                                                                \
    continue;                                                               \
  }
