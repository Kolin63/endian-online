...
#define END_JSON_CHECK_STRING                                              \
  if (iter->json->type != cJSON_String) {                                  \
    log_error("In command %s from mod %s, %s must be string", cmd_name,    \
              mod_name, iter->json->string);                               \
    error++;                                                               \
    continue;                                                              \
  }                                                                        \
  if (iter->json->valuestring == NULL) {                                   \
    log_error("In command %s from mod %s, %s is NULL", cmd_name, mod_name, \
              iter->json->string);                                         \
    error++;                                                               \
    continue;                                                              \
  }
...
#define END_JSON_CHECK_STRING_LENGTH(min, max)                           \
  {                                                                      \
    ...
    if (len < min || len > max) {                                        \
      log_error("In command %s from mod %s, %s is wrong size", cmd_name, \
                mod_name, iter->json->string);                           \
      error++;                                                           \
      continue;                                                          \
    }                                                                    \
...
#define END_JSON_CHECK_ARRAY                                             \
  if (iter->json->type != cJSON_Array) {                                 \
    log_error("In command %s from mod %s, %s must be array", cmd_name,   \
              mod_name, iter->json->string);                             \
    error++;                                                             \
    continue;                                                            \
  }                                                                      \
  if (iter->json->child == NULL) {                                       \
    log_error("In command %s from mod %s, %s has no children", cmd_name, \
              mod_name, iter->json->string);                             \
    error++;                                                             \
    continue;                                                            \
  }
