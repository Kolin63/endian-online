...
struct discord_application_command_options* command_options_end_to_conc(
    const struct command_options* end) {
  if (end == NULL) return NULL;

  struct discord_application_command_options* conc =
      malloc(sizeof(struct discord_application_command_options));
  conc->size = end->size;
  conc->array =
      malloc(sizeof(struct discord_application_command_option) * conc->size);
...
int command_option_choices_fillout(
    const char* mod_name, const char* file_name, const cJSON* json,
  ...
  while (true) {
    ...
    if (iter->json->type == cJSON_Object) {
      ...
      choices->size++;
      choices->array = reallocarray(
          choices->array, choices->size,
          sizeof(struct discord_application_command_option_choice));
      struct discord_application_command_option_choice* i =
          &(choices->array[choices->size - 1]);
      i->name = NULL;
      i->value = NULL;
    } else if (iter->json->type == cJSON_String) {
      END_JSON_CHECK_STRING;
      END_JSON_CHECK_STRING_LENGTH(1, 100);
      if (choices->size == 0 || iter->parent == NULL ||
          iter->parent->parent == NULL) {
        log_error("In command %s from mod %s, %s must be object", file_name,
                  mod_name, json->string);
        error++;
        continue;
      }
      ...
      if (i->name != NULL || i->value != NULL) {
        log_error("In command %s from mod %s, choices is wrong size", file_name,
                  mod_name, json->string);
        error++;
        continue;
      }
      ...
      if (strcmp(iter->json->string, "name") == 0) {
        ...
        log_error(
            "Command choice from command %s from mod %s has unknown object %s",
...
int command_option_channel_types_fillout(const char* mod_name,
  ...
  while (true) {
    ...
    if (strcmp(val, "GUILD_TEXT") == 0) {
      type = 0;
    } else if (strcmp(val, "DM") == 0) {
      type = 1;
    } else if (strcmp(val, "GUILD_VOICE") == 0) {
      type = 2;
    } else if (strcmp(val, "GROUP_DM") == 0) {
      type = 3;
    } else if (strcmp(val, "GUILD_CATEGORY") == 0) {
      type = 4;
    } else if (strcmp(val, "GUILD_ANNOUNCEMENT") == 0) {
      type = 5;
    } else if (strcmp(val, "ANNOUNCEMENT_THREAD") == 0) {
      type = 10;
    } else if (strcmp(val, "PUBLIC_THREAD") == 0) {
      type = 11;
    } else if (strcmp(val, "PRIVATE_THREAD") == 0) {
      type = 12;
    } else if (strcmp(val, "GUILD_STAGE_VOICE") == 0) {
      type = 13;
    } else if (strcmp(val, "GUILD_DIRECTORY") == 0) {
      type = 14;
    } else if (strcmp(val, "GUILD_FORUM") == 0) {
      type = 15;
    } else if (strcmp(val, "GUILD_MEDIA") == 0) {
      type = 16;
    } else {
      log_error("In command %s from mod %s, unknown channel type %s", file_name,
                mod_name, val);
      error++;
      continue;
    }
    ...
    channel_types->size++;
    channel_types->array =
        reallocarray(channel_types->array, channel_types->size, sizeof(int));
...
int command_option_fillout(const char* mod_name, const char* file_name,
  ...
  loop_no_iterate:
    ...
    if (strcmp(item_name, "type") == 0) {
      END_JSON_CHECK_STRING;
      const char* val = iter->json->valuestring;
      if (strcmp(val, "SUB_COMMAND") == 0)
        opt->type = DISCORD_APPLICATION_OPTION_SUB_COMMAND;
      else if (strcmp(val, "SUB_COMMAND_GROUP") == 0)
        opt->type = DISCORD_APPLICATION_OPTION_SUB_COMMAND_GROUP;
      else if (strcmp(val, "STRING") == 0)
        opt->type = DISCORD_APPLICATION_OPTION_STRING;
      else if (strcmp(val, "INTEGER") == 0)
        opt->type = DISCORD_APPLICATION_OPTION_INTEGER;
      else if (strcmp(val, "BOOLEAN") == 0)
        opt->type = DISCORD_APPLICATION_OPTION_BOOLEAN;
      else if (strcmp(val, "USER") == 0)
        opt->type = DISCORD_APPLICATION_OPTION_USER;
      else if (strcmp(val, "CHANNEL") == 0)
        opt->type = DISCORD_APPLICATION_OPTION_CHANNEL;
      else if (strcmp(val, "ROLE") == 0)
        opt->type = DISCORD_APPLICATION_OPTION_ROLE;
      else if (strcmp(val, "MENTIONABLE") == 0)
        opt->type = DISCORD_APPLICATION_OPTION_MENTIONABLE;
      else if (strcmp(val, "NUMBER") == 0)
        opt->type = DISCORD_APPLICATION_OPTION_NUMBER;
      else if (strcmp(val, "ATTACHMENT") == 0)
        opt->type = DISCORD_APPLICATION_OPTION_ATTACHMENT;
      else {
      ...
      END_JSON_CHECK_ARRAY_AND_CHILDREN;
      opt->choices =
          malloc(sizeof(struct discord_application_command_option_choices));
      opt->choices->array = NULL;
      opt->choices->size = 0;
      error += command_option_choices_fillout(mod_name, file_name, iter->json,
                                              opt->choices);
      goto loop_no_iterate;
      ...
      error += command_option_channel_types_fillout(
          mod_name, file_name, iter->json, opt->channel_types);
      goto loop_no_iterate;
    } else {
      log_error("In command %s from mod %s, unknown object %s", file_name,
...
int command_options_fillout(const char* mod_name, const char* file_name,
  ...
  if (json->type != cJSON_Array) {
    log_error("In command %s from mod %s, %s must be array", file_name,
              mod_name, json->string);
    error++;
    return error;
  }
  if (json->child == NULL) {
    log_error("In command %s from mod %s, %s has no children", file_name,
              mod_name, json->string);
    error++;
    return error;
  }
  ...
  while (i != NULL) {
    if (i->type != cJSON_Object) {
      log_error("In option %s from mod %s, %s must be object", file_name,
                mod_name, i->string);
      error++;
      continue;
    }
    if (i->child == NULL) {
      log_error("In option %s from mod %s, %s has no children", file_name,
                mod_name, i->string);
      error++;
      continue;
    }
    ...
    if (this_error == 0) {
      opts->size++;
      opts->options = reallocarray(opts->options, opts->size,
...
void command_load(const struct discord_ready* event, const char* command_path,
  ...
  struct discord_application_command_options* discord_opts =
      command_options_end_to_conc(params.options);
...
void command_options_cleanup(struct command_options* opts) {
  for (int i = 0; i < opts->size; i++) {
    ...
    for (int j = 0; j < opt->choices->size; j++) {
      struct discord_application_command_option_choice* c =
          &(opt->choices->array[j]);
      free(c->name);
      free(c->value);
    }
